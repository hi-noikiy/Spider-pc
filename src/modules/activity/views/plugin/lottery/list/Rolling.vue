<template>
  <div >
    ...
    <el-dialog title="提取链接" :visible.sync="simpleVisible" :append-to-body="true" :close-on-click-modal="false" :before-close="closeSimple" width="880px">
  ...
  data() {
    return {
      ...
      menu: [
        ...
        { label: '删除', type: 'danger', disabled: ({ currentRow }) => currentRow.id == undefined, on: this.deleteConfirm },
        { label: '查看参与嘉宾', type: 'primary', disabled: ({ currentRow }) => currentRow.id == undefined, on: () => this.jumpPage(1) },
        { label: '查看中奖名单', type: 'primary', disabled: ({ currentRow }) => currentRow.id == undefined, on: () => this.jumpPage(2) },
        { label: '查看弹幕', type: 'primary', disabled: ({ currentRow }) => currentRow.id == undefined, on: () => this.jumpPage(3) },
        { label: '提取链接', type: 'primary', disabled: ({ currentRow }) => currentRow.id == undefined, on: this.getVisible },
  ...
  methods: {
    ...
    jumpPage(type) {
      switch(type) {
        case 1:
          // 参与嘉宾
          this.$router.push(`/main/activity/plugin/lottery/member?id=${this.tableRow.id}&sourceId=${this.tableRow.sourceId}&sourceType=${this.tableRow.sourceType}&showType=1`)
          break;
        case 2:
          // 中奖名单
          this.$router.push(`/main/activity/plugin/lottery/winning?id=${this.tableRow.id}&sourceType=${this.tableRow.sourceType}`)
          break;
        case 3:
          // 查看弹幕
