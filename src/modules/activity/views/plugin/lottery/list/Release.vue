<template>
  <!-- <div class="wrap" ref="wrap" v-loading="dataLoading"> -->
    ...
    <div class="context-box" :style="{'height': `${areaHeight}px`}" ref="context">
      <el-form class="form" :model="model" ref="base" label-position="left" label-width="100px" size="small">
        ...
        <el-form-item label="活动名称" prop="title" :rules="[{ required: true, message: '该字段不能为空'}]">
          <el-input size="small" v-model="model.title" style="width:400px;" :maxlength="24" placeholder="建议10字内（不超过24字）"></el-input>
        </el-form-item>
        ...
        <el-form-item v-if="model.sourceType === 1 || model.sourceType === 2" prop="sourceId" :rules="[{ required: true, message: '该字段不能为空'}]">
        ...
        <el-form-item v-if="model.sourceType === 3" prop="">
          ...
          <up-load-file :id="model.id" :data="{id: model.id}" title="导入" url="api/admin/v1/lottery/import" icon="el-icon-upload" @file-success="fileSuccess" style="margin: 0 20px 20px 0;"></up-load-file>
        ...
        <el-form-item label="活动密码" prop="password" :rules="[{ required: true, message: '该字段不能为空'}]">
          <el-input size="small" type="text" v-model.trim="model.password" style="width:220px;" maxlength="32" placeholder="打开活动的密码"></el-input>
        </el-form-item>
        ...
        <table class="property-table item_error">
          ...
          <tr v-for="(item, i) in model.itemList" :key="i">
            ...
            <td style="width: 100px;">
              <el-form-item label-width="0px" :prop="`itemList[${i}].seqNum`" :rules="[{ required: true, message: '该字段不能为空'}]">
                <el-input-number :min="1" size="small" v-model="item.seqNum" style="width: 100px;"></el-input-number>
              </el-form-item>
            </td>
            <td>
              <el-form-item label-width="0px" :prop="`itemList[${i}].title`" :rules="[{ required: true, message: '该字段不能为空'}]">
                <el-input size="small" v-model.trim="item.title" :maxLength="16" placeholder="建议10字内；不超过16字；"></el-input>
              </el-form-item>
            </td>
            <td>
              <el-form-item label-width="0px" :prop="`itemList[${i}].awardNum`" :rules="[{ required: true, message: '该字段不能为空'}]">
                <el-input-number :min="1" :disabled="item.awardType === 'lose'" size="small" v-model="item.awardNum" style="width: 120px;"></el-input-number>
              </el-form-item>
            </td>
            ...
            <td v-if="model.raffledType == 'guest'">
              <el-form-item label-width="0px" :prop="`itemList[${i}].rate`" :rules="[{ required: true, message: '该字段不能为空'}]">
            ...
            <td @click="showImgBox(true, 'item', i)">
              <el-form-item label-width="0px" :prop="`itemList[${i}].imgUrl`" :rules="[{ required: true, message: '该字段不能为空'}]">
        ...
        <el-form :inline="true" label-position="top">
          <el-form-item label="活动标题" prop="logoUrl" :rules="[{ required: false, message: '该字段不能为空'}]" style="margin-right: 20px;">
          ...
          <el-form-item label="背景图片" prop="bgImgUrl" :rules="[{ required: false, message: '该字段不能为空'}]" style="margin-right: 20px;">
    ...
    <el-dialog title="选择活动" width="800px" :visible.sync="dialogTable" @close="closeDialogTable" :append-to-body="true" :close-on-click-modal="false" :close-on-press-escape="false">
      <div style="border: 1px solid #ebeef5;">
        <QcTable :option="option" :search-form="searchForm" :showSearchButtons="false" :column="column" @row-click="rowClick"></QcTable>
      </div>
    ...
    <div class="bottom-btn">
      <el-button type="primary" @click="submit(1)" style="margin-left: 30px;" size="small" :loading="loading">保存</el-button>
    </div>

    <login-dialog :visible="digLogin" @close="closeLogin"></login-dialog>
    <edit-image :visible="digImgWrap" image-type='lottery' :cropper-option="cropperOption" :img-name="imgName" :on-close="showImgBox" :on-singleimg="getSingleimg" :is-cut="isCut"></edit-image>
  <!-- </div> -->
    </QcNavigationPage>
</template>
...
export default {
  ...
  methods: {
    ...
    detail(id) {
      this.$http.getLottery({ id: id }).then(res => {
        ...
        if (model[key].constructor === Array) {
          model[key].map(item => {
            for (let key in item) {
              ...
              if (key === 'id' || key === 'appId' || key === 'updateTime' || key === 'createTime' || key === 'isDeleted' || key === 'lotteryId') {
                delete item[key]
              }
            }
          })
