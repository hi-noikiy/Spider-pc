  ...
  <div class="wrap" ref="wrap">
    <div ref="search">
      <button-wrap>
        ...
        <el-button type="primary" size="small" :disabled="tableList.id == null" @click="toWinning()">查看中奖名单</el-button>
        <el-button type="primary" size="small" :disabled="tableList.id == null" @click="getVisible()">提取链接</el-button>
        <el-button type="primary" size="small" :disabled="tableList.id == null" @click="open()">打开链接</el-button>
      </button-wrap>
    </div>   
    <qc-table-old ref="table" :table-list="showData" :search="searchData" url="api/admin/v1/lottery/page"></qc-table-old>
    <el-dialog title="抽奖链接" :visible.sync="simpleVisible" :append-to-body="true" :close-on-click-modal="false" :before-close="closeSimple" width="880px">
...
export default {
  ...
  methods: {
    ...
    //参与嘉宾
    toMember() {
      this.$router.push(`/main/activity/plugin/lottery/member?id=${this.tableList.id}&sourceId=${this.tableList.sourceId}&sourceType=${this.tableList.sourceType}`)
    },
    //中奖名单
    toWinning() {
