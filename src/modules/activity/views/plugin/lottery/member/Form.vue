<template>
  <el-dialog title='选择中奖等级' :visible.sync="visible" width="880px" :append-to-body="true" :before-close="close" :close-on-click-modal="false" :close-on-press-escape="false">
    <div style="overflow: auto;">
      <el-radio-group v-model="lotteryItemId">
        <el-radio v-for="(item, i) in itemList" :key="i" :label="item.id" border>{{item.title}}（{{item.awardNum}}人）</el-radio>
      </el-radio-group>
    </div>
    <span slot="footer" class="dialog-footer">
      <el-button size="small" @click="close(false)">取 消</el-button>
      <el-button size="small" type="primary" @click="success()" :disabled="lotteryItemId === ''" :loading="loading">确 定</el-button>
    </span>
