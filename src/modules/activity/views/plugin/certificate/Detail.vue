      ...
      <el-table-column label="操作" align="right">
        <template slot-scope="scope">
          <el-button
            ...
            @click="certificateDataGet(scope.row.id)"
          >修改</el-button>
          <el-button
            type="text"
            style="margin-right:10px"
            @click="certificateDataDelete(scope.row.id)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="证书信息" :visible="certificateData.visible" width="500px" :show-close="false" append-to-body>
      ...
      >
        ...
        <el-form-item
          ...
          :rules="[{required: true, message: '证书名称不能为空'},{max: 15, message: '证书名称不能超过15个字'}]"
        >
        ...
        <el-form-item
          label="颁奖日期："
          prop="awardTime"
          :rules="[{required: true, message: '颁奖日期不能为空'}]"
        >
          <el-date-picker
            type="date"
            format="yyyy-MM-dd"
...
export default {
  ...
  methods: {
    ...
    onDownload() {
      console.log(this.base.baseHost);
      console.log(this.base.projectName());
      location.href =
        this.base.baseHost +
        this.base.projectName() +
        "/api/admin/v1/courseCertificateData/exportDataTemplate";
    },
    certificateDataPage() {
