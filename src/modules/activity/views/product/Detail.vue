<template>
  <div class="wrap" ref="wrap" v-loading="dataLoading" element-loading-text="加载中...">
    ...
    <div
      ...
      >
      <QcNavigationPage :content="model.title || '报名详情'" :full="false" background="transparent">
        <el-menu
          mode="horizontal"
        ...
        <div class="content">
          ...
          <div v-show="activeIndex === '2'">
            ...
            <div class="check-time" v-if="model.activityCheckinSetting">
              <el-table :data="model.activityCheckinSetting.checkinTimeList" @row-click="listCheckin">
                <el-table-column label="签到开始时间" prop="checkinStartTime"></el-table-column>
                <el-table-column label="签到结束时间" prop="checkinEndTime"></el-table-column>
                <el-table-column label="签到迟到时间" prop="checkinLateTime"></el-table-column>
                <el-table-column label="签到二维码">
                  <template slot-scope="scope">
                    <el-popover
                      v-if="(!$store.state.appTypeList.length || $store.state.appTypeList.some(item => item === 2)) && scope.row.checkinQrcodeUrl"
                      placement="right"
                      width="300"
                      trigger="click"
                    >
                      <img :src="imgHost + scope.row.checkinQrcodeUrl" alt style="width: 100%;" />
                ...
                >
                  <template slot-scope="scope">
                    <el-popover
                      v-if="(!$store.state.appTypeList.length || $store.state.appTypeList.some(item => item === 2)) && scope.row.checkinSettingId"
                      placement="right"
                      width="300"
                      trigger="click"
                    >
                    ...
                    <WxQRCode
                      ...
                      size="mini"
                      :options="{ name: 'mobileCheckin', scene: scope.row.checkinSettingId }"
                    ></WxQRCode>
                  </template>
                </el-table-column>
                <el-table-column label="暗号二维码" v-if="model.activityCheckinSetting.checkinCode">
                  <template slot-scope="scope">
                    <el-popover
                      v-if="(!$store.state.appTypeList.length || $store.state.appTypeList.some(item => item === 2)) && scope.row.checkinSettingId"
                      placement="right"
                    ...
                    <WxQRCode
                      v-if="$store.state.appTypeList.some(item => item === 3) && scope.row.checkinSettingId"
                      size="mini"
                      :options="{ name: 'codeCheckin', scene: scope.row.checkinSettingId }"
                    ></WxQRCode>
                  </template>
                </el-table-column>
                <el-table-column label="空降嘉宾" v-if="model.activityCheckinSetting.isEnableAirborne">
                  <template slot-scope="scope">
                    <el-popover
                      v-if="(!$store.state.appTypeList.length || $store.state.appTypeList.some(item => item === 2)) && scope.row.checkinSettingId"
                      placement="right"
                    ...
                    <WxQRCode
                      v-if="$store.state.appTypeList.some(item => item === 3) && scope.row.checkinSettingId"
                      size="mini"
                      :options="{ name: 'airborneCheckin', scene: scope.row.checkinSettingId }"
                    ></WxQRCode>
                  </template>
                </el-table-column>
              </el-table>
                ...
                <el-form :inline="true">
                  ...
                  <el-form-item>
                    <el-button
                      ...
                      "
                    >查询</el-button>
                    <el-button type="primary" size="small" @click="checkExportData()">导出Excel</el-button>
                  </el-form-item>
                </el-form>
              </div>
            </div>
    ...
    <el-dialog
      ...
      title="线下支付"
      width="400px">
      <el-form size="small" :model="payModel" label-width="100px" label-position="left">
        <el-form-item label="支付方式">
          <el-select v-model="payModel.payChannel" style="width:100%">
            <el-option
      ...
      <span slot="footer">
        <el-button size="small" @click="onPayClose()">关闭</el-button>
        <el-button size="small" type="primary" @click="payCharge()">确定</el-button>
      </span>
    </el-dialog>
    <el-dialog
    ...
    return {
      ...
      downloadTaskData: {
        ...
        propData: [
          ...
          { prop: 'size', label: '文件大小', formatter: row => {
            if (!row.size) return ''
