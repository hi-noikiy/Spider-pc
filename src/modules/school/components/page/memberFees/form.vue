  ...
  methods: {
    ...
    async pageMemberLevel(id) {
      try {
        ...
        let selectList = this.memberLevelList.filter(item => this.model.sourceIds.indexOf(item.id) != -1)
    ...
    saveCurrent() {
      if (this.dialogType === 1) {
        ...
        let selectList = this.tableRow.filter(item => this.model.sourceIds.indexOf(item.id) === -1)
