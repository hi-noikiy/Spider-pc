    ...
    <el-form :model="model" ref="model" label-position="right" label-width="100px">
      <QcTitle title="基础信息">
        <el-form-item label="商品" prop="productId" :rules="[{required:true,message:'请填写该字段'}]">
          ...
          <el-button type="primary" size="small" :disabled="model.id ? true : false" @click="dialogVisible = true">关联产品</el-button>
        ...
        <el-form-item v-if="false" label="推广积分" prop="sharerPoint" :rules="[{required:true,message:'请填写该字段'}]">
          <el-input class="form-item__input" v-model="model.sharerPoint"></el-input>
        </el-form-item>
        <el-form-item v-if="false" label="销售佣金" prop="distributerCommission" :rules="[{required:true,message:'请填写该字段'}]">
          <el-input class="form-item__input" v-model="model.distributerCommission"></el-input>
        </el-form-item>
        <el-form-item v-if="false" label="销售积分" prop="distributerPoint" :rules="[{required:true,message:'请填写该字段'}]">
...
export default {
  ...
  methods: {
    ...
    addSharerShopping(model) {
      this.$http.addSharerShopping(model).then(() => {
    ...
    updateSharerShopping(model) {
      this.$http.updateSharerShopping(model).then(() => {
        this.$message.success('保存成功，2秒后将进行跳转')
        setTimeout(() => {
          this.$router.back()
        }, 1500)
