      ...
      >
        ...
        <div v-if="model.isTabBar">
          <el-form-item label="未选中图标" prop="iconPath">
            <QcImageUpload
              :data="{ imageType: 'component' }"
              v-model="model.iconPath"
            ></QcImageUpload>
          </el-form-item>
          <el-form-item label="选中图标" prop="selectedIconPath">
            <QcImageUpload
              :data="{ imageType: 'component' }"
              v-model="model.selectedIconPath"
            ></QcImageUpload>
          </el-form-item>
        </div>
      ...
      column: [
        ...
        },
        {
          ...
          formatter: row => {
            return (
              <el-image
                src={this.imgHost + row.selectedIconPath}
                style="width:60px;height:60px"
                fit="cover"></el-image>
            )
          }
          ...
          formatter: row => {
            return (
              <div>
                ...
                <el-button
                  type="text"
                  onClick={this.deleteFlyWheelNavigationBar.bind(this, row.id)}>
                  删除
                </el-button>
              </div>
            )
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.model.validate(val => {
        if (val) {
          this.model.id
            ? this.updateFlyWheelNavigationBar(this.model)
            : this.addFlyWheelNavigationBar(this.model)
        }
      })
    },
