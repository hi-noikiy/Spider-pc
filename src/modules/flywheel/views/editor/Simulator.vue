<template>
  <div class="simulator">
    ...
    >
      <img src="./img/status_bar.png" width="100%" />
      <img src="./img/menu.png" class="simulator__header-menu" />
      <p class="title">{{ currentPage.navigationBarTitleText }}</p>
    </header>
    <section class="simulator__main" :style="{ background: currentPage.background }">
      <div
      ...
      <div class="simulator__move">
        <div v-if="currentPage.isSystem">
          ...
          <img v-else :src="imgHost + currentPage.pageImg" width="100%" />
        </div>
        <template v-else v-for="component in simulatorComponent">
          <div
            class="comp-wrap"
            :key="component.id"
          ...
          >
            <div class="border" :class="{ 'border-active': component.id == currentComponent.id }"></div>
            <i class="comp-icon el-icon-remove" @click.stop="removeComponentToSimulator(component.id)"></i>
            <component :is="component.name" :options="component.options"></component>
          </div>
        </template>
        <template v-for="component in simulatorComponent">
          ...
          >
            <div style="position: relative;width: 45px;height: 45px;">
              <i style="position: absolute;z-index:10000;color:#ff5433;right:-5px;top:-5px" class="el-icon-remove" @click.stop="removeComponentToSimulator(component.id)"></i>
              <component
      ...
      >
        <template v-for="page in pageList">
          <div class="tab" v-if="page.isShow" :key="page.id" @click="updateCurrentPageId(page.id)">
            <img
