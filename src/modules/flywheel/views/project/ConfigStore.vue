<template>
  <div class="app-project">
    <div class="app-project__item app-wrap">
      ...
      <div>
        <el-button type="text" @click="uploadProject()">上传项目</el-button><br />
      </div>
    </div>
    ...
    <div class="app-project__item" v-if="auditModel">
      ...
      <div class="app-project__item-wrap">
        ...
        <el-form style="flex: 500px 0 1;" label-position="left" label-width="85px">
          ...
          <el-form-item label="描述" style="margin-bottom:0">{{ auditModel.userDesc }}</el-form-item>
    ...
    <!-- 版本描述 -->
    <el-dialog :visible="versionVisible" append-to-body title="信息描述" :show-close="false" width="450px">
...
import createStoreExt from './storeExt'
export default {
  components: {
    FeedbackDialog,
    PreviewDialog,
  ...
  methods: {
    async getWxAppConfig() {
      let res = await this.$http.getWxAppConfig({purpose: 'mall-store'})
    ...
    async commitCode(id, templateId) {
      ...
      try {
        let storeExtData = createStoreExt(this.model)
        const commitModel = {
        ...
        const { extJson, userVersion, userDesc, path } = commitModel
        // 注意区分是店铺端小程序还是门店端
    ...
    submitAudit() {
      ...
      this.$confirm('项目将提交到微信，进行审核。这通常需要1~7天。', '提交审核', {
        ...
        .then(() => {
          this.$http.submitAudit({purpose: 'mall-store'}).then(() => {
    ...
    speedAudit() {
      this.$http.getLatestAuditstatus({purpose: 'mall-store'}).then(res => {
        if (res.data.data.code !== 0) {
          this.$confirm('加急成功后，微信将会在2~12小时内处理你的审核请求。是否执行该操作', '操作提示', {
            ...
            .then(() => {
              this.$http.speedAudit({ auditId: res.data.data.auditid, purpose: 'mall-store' }).then(() => {
    ...
    undoCodeAudit() {
      this.$confirm('每天审核撤回次数最多不超过 1 次，一个月不超过 10 次。', '次数限制，请谨慎操作', {
        ...
        .then(() => {
          this.$http.undoCodeAudit({purpose: 'mall-store'}).then(() => {
    ...
    releasePassCode() {
      this.$http.releasePassCode({purpose: 'mall-store'}).then(() => {
        this.$message.success('发布成功')
        this.updateThirdPartyCommitCode('AUDIT_RELEASE', 'audit')
      }).catch(err => {
        if (err && err.data && err.data.message && err.data.message.indexOf('发布已通过审核的小程序失败app is already released') != -1) {
          setTimeout(() => {
            this.$message.close()
    ...
    revertCode() {
      this.$http.revertCode({purpose: 'mall-store'}).then(() => {
    ...
    getThirdPartyCommitCode(code) {
      this.$http.getThirdPartyCommitCode({ code, purpose: 'mall-store' }).then(res => {
        if (res.data.data) {
