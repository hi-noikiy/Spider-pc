<template>
  <div class="radar-detail">
    ...
    <div class="container">
      <div class="overview">
        <div class="left">
          <div class="left-item"><span class="item-label">标题：</span><span>{{radarModel.title}}</span></div>
          <div class="left-item" v-if="radarModel.type === 'link'"><span class="item-label">链接：</span><span>{{radarModel.link}}</span></div>
          <div class="left-item" v-if="radarModel.type === 'file'"><span class="item-label">文件：</span><span>{{radarModel.fileName}}</span></div>
          <div class="left-item-row">
            <div class="row-col"><span class="item-label">创建时间：</span><span>{{radarModel.createTime}}</span></div>
            <div class="row-col"><span class="item-label">行为通知：</span><span>{{radarModel.isBehaviorNotice ? '已开启' : '已关闭'}}</span></div>
            <div class="row-col"><span class="item-label">动态通知：</span><span>{{radarModel.isActionNotice ? '已开启' : '已关闭'}}</span></div>
          </div>
          <div class="left-item"><span class="item-label">客户标签：</span>
            <template v-if="radarModel.behaviorTagList && radarModel.behaviorTagList.length > 0">
              <span class="item-tag" v-for="(item, index) in radarModel.behaviorTagList" :key="index">{{item.name}}</span>
            </template>
            <template v-else>
              <span>暂无</span>
            </template>
        ...
        <div class="query-table-box">
          <el-form inline label-width="80px" label-position="right" size="small">
            <el-form-item label="客户名称">
              <el-input placeholder="请输入要搜索的客户" v-model="searchForm.customerName" @input="searchOpenList"></el-input>
            </el-form-item>
            <el-form-item label="成员名称">
              <el-input placeholder="请输入要搜索的成员" v-model="searchForm.userName" @input="searchOpenList"></el-input>
            </el-form-item>
