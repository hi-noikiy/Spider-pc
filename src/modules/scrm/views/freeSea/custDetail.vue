<template>
  <div>
    <div class="header_nav" style="margin-bottom: 15px">
      <el-button type="text" icon="el-icon-arrow-left" @click="$router.go(-1)">客户详情</el-button>
    </div>
      ...
      <el-tabs type="border-card" class='clicontent' :style="{height:height+'px'}">
        <el-tab-pane label="跟进记录">
          <div>
            ...
            <div class='followup'>
              <el-input class='record' type="textarea" placeholder="添加跟进记录，1000字以内" maxlength="1000"></el-input>
            </div>

            <div class='followup' style='display: flex;flex-wrap:wrap;width:550px'>
              <div v-if='followImages.length>0' v-for="(pic,index) in followImages" :key="index" style='position:relative;margin-right:5px;margin-bottom:2px;'>
                <!-- <img :src="`${imgHost}${pic}`" style="width:100px;height:100px;"/> -->
                <el-image :src="`${pic}`" :preview-src-list="followImages" style="width:100px;height:100px;"></el-image>
                <i class='el-icon-circle-close' style='position:absolute;top:0px;right:0px;color:red' @click='deletefollow(index)'></i>
              </div>
        ...
        <el-tab-pane label="用户画像">
          <div class='int_box' v-if='!isedit'>
            <div style='margin-bottom:10px'><el-button size="small" type="primary" plain @click="isedit=true">编辑</el-button></div>
            <div class='portrayal' :style="{height:height-40-30-42+'px'}">
              <div class='portrow'><div>手机</div><div>暂无</div></div>
              <div class='portrow'><div>姓名</div><div>暂无</div></div>
              <div class='portrow'><div>性别</div><div>暂无</div></div>
              <div class='portrow'><div>生日</div><div>暂无</div></div>
              <div class='portrow'><div>年龄</div><div>暂无</div></div>
              <div class='portrow'><div>QQ</div><div>暂无</div></div>
              <div class='portrow'><div>邮箱</div><div>暂无</div></div>
              <div class='portrow'><div>爱好</div><div>暂无</div></div>
              <div class='portrow'><div>学历</div><div>暂无</div></div>
              <div class='portrow'><div>年收入</div><div>暂无</div></div>
              <div class='portrow'><div>行业</div><div>暂无</div></div>
              <div class='portrow'><div>公司</div><div>暂无</div></div>
              <div class='portrow'><div>区域</div><div>暂无</div></div>
              <div class='portrow'><div>地址</div><div>暂无</div></div>
              <div class='portrow'><div>身份证</div><div>暂无</div></div>
              <div class='portrow'><div>微信号</div><div>暂无</div></div>
              <div class='portrow'><div>图片</div><div>暂无</div></div>
            </div>
          </div>
          <div class='int_box' v-if='isedit'>
            <div style='margin-bottom:10px'>
              <el-button size="small" type="primary" plain @click="isedit=false">取消</el-button>
              ...
              <div class='portcol'><div class='picimg'>图片</div>
                <div class='loadpic'>
                  <div v-if='imagelist.length>0' v-for="(pic,index) in imagelist" :key="index" class='imgbox'>
                    ...
                    <el-image :src="pic" :preview-src-list="imagelist" style="width:50px;height:50px;"></el-image>
