    ...
    <div class="container">
      <div class="container-top">
        <div class="violation-notice">
          <i class="el-icon-circle-check" v-if="model.isShow == true"></i>
          <i class="el-icon-circle-close" v-if="model.isShow == false"></i>
          被删除提醒
          <el-switch v-model="model.isShow" @change="handleOpenStatus(model.isShow)"></el-switch>

          <div class="violation-notice-dispose">
            <el-alert
              title="开启后,当企业微信被客户删除时,被删除的企业成员将收到一条消息提醒"
              type="warning"
              show-icon
              :closable="false"
            >
  ...
  methods: {
    chooseItemAdmin(adminItem) {
      ...
      for (var i = 0; i < this.reChooseList.length; i++) {
        if (hash.indexOf(this.reChooseList[i]) == -1) {
      ...
      if (this.userId != '') {
        ...
        this.$http.getPermitUserPage(params).then(res => {
          ...
          for (var i = 0; i < this.chooseAdminList.length; i++) {
            if (msg.indexOf(this.chooseAdminList[i].userId) > -1) {
    ...
    //查看聊天记录
    openChatRecode(row) {
      this.$router.push(`/main/scrm/control/message/index?conversationId=${row.conversationId}&userId=${row.wxUserId}`)
    },
    //客户详情
    showCustomer(row) {
      console.log(row.id)
      this.$router.push(`/main/scrm/customer-management/customer/detail?id=${row.customerId}`)
    },
    //点击选择客户
    handleConfirm() {},
    //打开选择列表
