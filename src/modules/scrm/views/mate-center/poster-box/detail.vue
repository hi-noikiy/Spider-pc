<template>
  <QcNavigationPage content="添加海报" full>
        ...
        <el-form-item label="背景图片" prop="imgUrl" :rules="[{ required: true, message: '该字段不能为空' }]">
          ...
          >
            ...
            <div slot="tip" class="el-upload__tip">
              ...
              <div>图片大小不超过2M，支持JPG / png格式文件</div>
            </div>
          </el-upload>
        </el-form-item>
        ...
        <div v-if="showclient || showmember">
          <div v-for="(item, index) in model.list" :key="index" v-if="item.type == 'text'">
            <el-form-item label="文本">
              <el-input type="textarea" v-model="item.content" :maxlength="24"  placeholder="昵称不超过24个字符"></el-input>
            </el-form-item>
            <el-form-item label="字体样式">
              <span class="span">
                <span class="span"
                  >大小<el-input v-model="item.style.fontSize" value="12px" class="setting"></el-input
                ></span>
                <span class="span"
                  >行高<el-input v-model="item.style.lineHeight" value="14px" class="setting"></el-input
                ></span>
                <span class="span"
                  >字距<el-input v-model="item.style.letterSpacing" value="0px" class="setting"></el-input
                ></span>
        ...
        <div v-if="showadd">
          <div v-for="(item, index) in model.list" :key="index" v-if="item.type == 'img'">
            <el-form-item label="二维码样式">
              <span class="span">
                <span class="span">宽
                  <el-input v-model="item.style.width" value="12px" class="setting"></el-input>
                </span>
                <span class="span">高
...
export default {
  ...
  methods: {
    ...
    success(res,file, fileList){
      this.fileList=[]
      var url=this.iamgeurl+res.data.imageUrl
      console.log(url)
      this.model.imgUrl=url
      this.fileList.push({url:url,name:file.name})
      let oImg = this.$refs.grapost.$el.children[0];
      oImg.src= url;
      oImg.style.width= '330px';
    ...
    save () {
      ...
      html2canvas(this.$refs.grapost.$el,{scrollX:0,scrollY:0,width:width,height:height,backgroundColor: null,allowTaint: true,useCORS: true}).then(canvas => {
