    ...
    <div class="upload-btn">
      ...
      <div class="upload-box">
        <div class="upload-box__item">
          ...
          <el-button icon="el-icon-plus" :class="{greenColor: coverFile}">{{!coverFile ? '选择封面' : '已选择'}}</el-button>
        </div>
        <div class="upload-box__item">
          <input v-show="!loading" class="input" type="file" accept="video/*" @change="inputChange" />
          <el-button icon="el-icon-plus" :class="{greenColor: videoFile}">{{!videoFile ? '选择视频' : '已选择'}}</el-button>
        </div>
        <el-button v-show="!loading" icon="el-icon-upload" type="primary" @click="submitUpload">确认上传</el-button>
        <el-button 
          v-show="loading" 
          class="el-btn"
          icon="el-icon-upload" 
          type="primary" 
          :loading="loading" 
          @click="submitUpload">上传中 {{ (percent * 100).toFixed(0) }}%</el-button>
