<template>
  <el-form label-position="right" label-width="320px" style="margin:40px 100px">
    <el-form-item :label="storeJoinConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
      ...
      <el-button style="margin-left:10px" type="text" :loading="storeJoinConfig.loading" @click="updateStoreJoinConfig()">保存</el-button>
      <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="开启后店铺小程序登录页面会显示 '店铺入驻' 按钮">
    ...
    <el-form-item :label="authMultiStoreConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
      ...
      <el-button style="margin-left:10px" type="text" :loading="authMultiStoreConfig.loading" @click="updateAuthMultiStoreConfig()">保存</el-button>
      <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="多店铺账号请开启，开启后PC端会显示:授权店铺小程序按钮，店铺小程序飞轮魔盒">
        <i class="el-icon-question popover-icon" slot="reference"></i>
      </el-popover>
    </el-form-item>
    <el-form-item :label="mallTypeConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
      ...
      <el-button style="margin-left:10px" type="text" :loading="mallTypeConfig.loading" @click="updateMallTypeConfig()">保存</el-button>
      <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="多店铺账号请开启，开启后小程序端购物车，订单列表、详情等页面，会显示店铺信息">
        <i class="el-icon-question popover-icon" slot="reference"></i>
      </el-popover>
    </el-form-item>
    <el-form-item :label="productVipPriceConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
      ...
      <el-button style="margin-left:10px" type="text" :loading="productVipPriceConfig.loading" @click="updateProductVipPriceConfig()">保存</el-button>
      <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="开启后商品详情将显示VIP价格，会员下单时自动使用VIP价格">
        <i class="el-icon-question popover-icon" slot="reference"></i>
      </el-popover>
    </el-form-item>
    <el-form-item :label="productVipNameConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
      <el-input
        ...
        show-word-limit
        clearable
        style="width: 120px;padding-left: 20px;"></el-input>
      <el-button style="margin-left:10px" type="text" :loading="productVipNameConfig.loading" @click="updateProductVipNameConfig()">保存</el-button>
      <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="例如输入‘会员价’ 页面将显示 ‘会员价￥99.00’">
        <i class="el-icon-question popover-icon" slot="reference"></i>
      </el-popover>
    </el-form-item>
  </el-form>
