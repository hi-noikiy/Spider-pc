      ...
      <div v-loading="loading">
        <el-form ref="model" :model="model" size="small" label-position="right" label-width="120px">
          ...
          <el-form-item label="使用范围" prop="couponScope" :rules="[{ required: true, message: '该项为必填项' }]">
            <el-radio v-model="model.couponScope" :label="101">所有商品</el-radio>
            <el-radio v-model="model.couponScope" :label="201">指定店铺</el-radio>
          </el-form-item>
          <el-form-item
  ...
  data() {
    return {
      ...
      model: {
        ...
        // 优惠券标题
        couponScope: 101,
        // 101 所有商品（平台用）  201 指定店铺（平台用） 204 指定商品（店铺用）
        couponType: 1,
        // 优惠券类型：1抵扣2折扣
      ...
      column: [
        ...
        },
        {
          ...
          formatter: row => {
            return (
              <div>
                {row.couponOrderAmount == 0 ? '无门槛' : '满' + row.couponOrderAmount / 100} 减 {row.couponAmount / 100}
              </div>
            )
          }
        },
          ...
          formatter: row => {
            return (
              <div>
                ...
                {!row.isPublished &&
                  (!row.isAborted || new Date(row.couponExpireTime).getTime() < new Date().getTime()) && (
    ...
    saveModel(type, model) {
      couponApi[type](model)
        .then(() => {
          this.loading = false
          this.$message({
            message: type === 'updateCoupon' ? '修改成功' : '添加成功',
            type: 'success'
          })
          this.close(true)
        })
