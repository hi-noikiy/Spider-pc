...
export default {
  ...
  computed: {
    isStoreRole() {
      // 计算属性不能在v-if上用？？？
      if (this.$store.state.isStoreRole === null || this.$store.state.isStoreRole === undefined || this.$store.state.isStoreRole === 'undefined') {
        // 判断当前登录账号是否是店铺角色
  ...
  methods: {
    ...
    orderAfterSalesExchangeGoodsRefuse() {
      this.$prompt('', '是否拒绝买家的换货申请？', {
        ...
        .then(({ value }) => {
          this.$http.orderAfterSalesExchangeGoodsRefuse({ afterSaleOrderId: this.model.id, refuseReason: value }).then(() => {
            this.$message.success('操作成功')
            this.getServiceOrder()
          })
        })
    ...
    orderAfterSalesExchangeGoodsConfirmCancel() {
      this.$prompt('', '是否取消买家的换货申请？', {
        ...
        .then(({ value }) => {
          this.$http.orderAfterSalesExchangeGoodsConfirmCancel({ afterSaleOrderId: this.model.id, cancelReason: value }).then(() => {
            this.$message.success('操作成功')
            this.getServiceOrder()
          })
        })
    ...
    orderAfterSalesReturnGoodsRefuse() {
      this.$prompt('', '是否拒绝买家的退货申请？', {
        ...
        .then(({ value }) => {
          this.$http.orderAfterSalesReturnGoodsRefuse({ afterSaleOrderId: this.model.id, refuseReason: value }).then(() => {
            this.$message.success('操作成功')
            this.getServiceOrder()
          })
        })
    ...
    orderAfterSalesReturnGoodsCancel() {
      this.$prompt('', '是否取消买家的退货申请？', {
        ...
        .then(({ value }) => {
          this.$http.orderAfterSalesReturnGoodsCancel({ afterSaleOrderId: this.model.id, cancelReason: value }).then(() => {
            this.$message.success('操作成功')
            this.getServiceOrder()
          })
        })
