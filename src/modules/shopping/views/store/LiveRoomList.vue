<template>
  <div>
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="直播背景图" align="center">
          <template slot-scope="scope">
            <el-image
              :src="imgHost + scope.row.roomCoverUrl"
              fit="cover"
        ...
        <el-table-column label="允许评论" align="center">
          <template slot-scope="scope">
            {{ scope.row.isAllowComment ? '是' : '否' }}</template
          ></el-table-column
        >
        ...
        <el-table-column label="操作" align="right">
          <template slot-scope="scope">
            <div v-if="scope.row.auditStatus == 0">
              <el-button
                type="text"
                @click="$router.push('/main/mall/store/room/data?id=' + scope.row.id)"
              >
                直播概况
              </el-button>
    ...
    <el-dialog
      ...
      ><div>
        <el-button @click="wxRoomRefresh()" type="primary" :loading="refreshLoading"
          >刷新直播间</el-button
        >
      </div>
      <QcTable
    ...
    <el-dialog :visible="visible" append-to-body title="通过审核" width="450px" :show-close="false">
      <el-form ref="model" :model="model" label-width="120px" label-position="left">
        <el-form-item
          label="关联直播间"
          prop="wxliveRoomId"
          :rules="[{ message: '请关联直播房间', required: true }]"
        >
          <el-button type="primary" size="mini" @click="wxRoomDialogVisible = true"
            >选择直播间</el-button
          >
        ...
        <el-form-item
          label="关联频道"
          prop="liveChannelId"
          :rules="[{ message: '请关联频道', required: true }]"
        >
          <el-select v-model="model.liveChannelId">
            <el-option
              v-for="item in liveChannelList"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
