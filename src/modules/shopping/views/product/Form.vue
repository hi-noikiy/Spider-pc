    ...
    <el-form :model="model" ref="productModel" label-position="right" label-width="auto">
      <QcTitle title="基本信息">
        ...
        >
          ...
          <el-cascader
            ...
            :options="categoryLists"></el-cascader>
        </el-form-item>
        ...
        <el-form-item v-if="false" label="自定义参数">
          <div class="property-box">
            ...
            <div
              ...
              class="property-item"
              v-for="(item, i) in model.productSpecParams"
              :key="i"
            >
      ...
      <QcTitle title="库存信息">
              ...
              <tr>
                <th v-for="(item, i) in tableTitleComputed" :key="i">
                  <div class="input-group" v-if="item.inputType">
                    <el-input :placeholder="item.title" 
                      style="width:150px" 
                      v-model="item.value" 
                      @keyup.native="['origPrice', 'price', 'vipPrice'].includes(item.inputType) ? checkInputPrice($event, item, 'value') : checkInputInt($event, item, 'value')">
                      <el-button
                        slot="append"
                        type="primary"
                        icon="el-icon-check"
                ...
                <template v-for="(label, index) in tableBodyValuesComputed(item.itemValue)">
                  ...
                  >
                    <el-input
                      ...
                      @blur="compareValue"
                      @keyup.native="['origPrice', 'price', 'vipPrice'].includes(label.inputType) ? checkInputPrice($event, label, 'value') : checkInputInt($event, label, 'value')"
                    ></el-input>
                    <span
                      v-else
        ...
        <el-form-item label="原价" prop="origPrice" :rules="[{ required: true, message: '该字段为必填项' }]">
          <el-input :disabled="model.isMultiSpec" v-model="model.origPrice" @keyup.native="checkInputPrice($event, model, 'origPrice')" style="width:450px;"></el-input>
        </el-form-item>
        <el-form-item label="现价" prop="price" :rules="[{ required: true, message: '该字段为必填项' }]">
          <el-input :disabled="model.isMultiSpec" v-model="model.price" @keyup.native="checkInputPrice($event, model, 'price')" style="width:450px;"></el-input>
        </el-form-item>
        <el-form-item label="会员价" prop="vipPrice" v-if="isOpenVipPrice" :rules="[{ required: true, message: '该字段为必填项' }]">
          <el-input :disabled="model.isMultiSpec" v-model="model.vipPrice" @keyup.native="checkInputPrice($event, model, 'vipPrice')" style="width:450px;"></el-input>
        </el-form-item>
        <el-form-item label="库存" prop="qty" :rules="[{ required: true, message: '该字段为必填项' }]">
          <el-input :disabled="model.isMultiSpec" v-model="model.qty" style="width:450px;" @keyup.native="checkInputInt($event, model, 'qty')"></el-input>
        </el-form-item>
        <el-form-item
          label="最低起订量"
