<template>
  <div>
    <QcTable ref="table" :option="option" :menu="menu" :search-form="searchForm" @get-list="onGetList">
      <div>
        <el-tabs v-model="searchForm[0].value" @tab-click="onSearch" type="card" style="margin:0 15px">
          <el-tab-pane :label="`全部(${count.total || 0})`" :name="' '"></el-tab-pane>
          <el-tab-pane :label="`待付款(${count.pay || 0})`" name="0"></el-tab-pane>
          <el-tab-pane :label="`待发货(${count.unDeliver || 0})`" name="1"></el-tab-pane>
          <el-tab-pane :label="`已发货(${count.deliver || 0})`" name="2"></el-tab-pane>
          <el-tab-pane :label="`已取消(${count.cancel || 0})`" name="-1"></el-tab-pane>
          <el-tab-pane :label="`已完成(${count.finish || 0})`" name="10"></el-tab-pane>
        </el-tabs>
            ...
            <tbody>
              <tr style="background: #ebeef5;">
                <td class="bb1" colspan="7" style="text-align:left;">
                  店铺：{{item.storeName || '--'}}&nbsp;&nbsp;&nbsp;订单编号：{{ item.orderNo || '--' }}&nbsp;&nbsp;&nbsp;微信单号：{{ item.tradeNo || '--' }}
                </td>
              ...
              <tr>
                <td colspan="8" style="border-top: 1px solid #ebeef5;text-align: left;padding-left: 15px;">
                  买家留言：{{ item.remark || '--' }}
                </td>
              </tr>
            </tbody>
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          成功：
          <span style="color: #67c23a">{{ deliver.model.successNum || 0 }}</span
          >&nbsp;条
        </el-col>
        <el-col :span="8">
          失败：
          <span style="color: #ff5433">{{ deliver.model.failNum || 0 }}</span
          >&nbsp;条
        </el-col>
      </el-row>
      <br />
