<template>
  <div class="source-data-wrap">
    <template v-if="valueType">
      <div style="margin-bottom: 10px;" v-for="(item, index) in value" :key="index">
        <div class="qc-data-source__item">
          ...
          <div class="qc-data-source__item-text">
            ...
            ></el-input>
            <div v-else class="text" style="background: #e8e8e847;color: #b9b9b9;">{{ item.name }}</div>
            <div class="text" @click="onQcProductPickerShow(index)"><i class="el-icon-paperclip" />&nbsp;关联产品</div>
          </div>
          <div class="qc-data-source__item-remove" @click="delSource(index)">
    ...
    ></QcProductPicker>
    <QcParamsDialog :visible="paramsVisible" :url="url" @close="onParamsVisibleClose"></QcParamsDialog>
  </div>
</template>
...
export default {
  props: {
    ...
    module: {
      type: Array,
      default: () => {
        return ['Mall', 'MallGroupShopping', 'ProductCategory', 'ProductLevel', 'XiaoeTechCourseVip', 'Activity', 'Lesson', 'News', 'Survey', 'Channel', 'Donate', 'DonateCategory']
      }
    },
    // 保存数据的格式
    value: {
